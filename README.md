...
### 🛰️ What AgriFieldNet actually contains

* **256×256 Sentinel-2 image chips**
...
│
├─ config/
│   └─ config.yaml                 # paths, device, tile_limit, class names/colors, band list
│
├─ data/                           # local data (NOT pushed)
│   ├─ raw/                        # downloaded AgriFieldNet (untouched)
│   │   ├─ source/
│   │   ├─ train_labels/
│   │   └─ test_labels/
│   │
│   ├─ processed/                  # PyTorch-ready (generated once)
│   │   ├─ train_images.npy         # (N, C, H, W)
│   │   ├─ train_masks.npy          # (N, H, W)
│   │   ├─ val_images.npy
│   │   ├─ val_masks.npy
│   │   └─ class_map.json
│   │
...
│   └─ exports/                    # PNG / CSV generated by backend
│
└─ tests/
   └─ test_smoke.py
```
...
## 2) Divide work across 5 students (balanced, parallel, minimal dependency) — **UPDATED (FastAPI + React)**
...
* Implement reusable components (`blocks.py`) including:
  ...
  * CNN blocks (Conv–BN–ReLU, down/up blocks)
...
* “Run Analysis” button → calls `POST /api/infer`
...
➡️ No student waits for another.

### Practical “mock-first” approach (so nobody blocks)

* Define a **shared API contract early**:

  ```text
  POST /api/infer
  → overlay_image (base64 or URL)
  → stats_table (JSON)
  → raw_mask (optional)
  ```
...
backend/
├─ api/         → Student D (FastAPI routes)
├─ services/    → Student D (connect API to ML)
└─ main.py      → backend entry point

frontend/
└─ src/         → Student E (React + shadcn UI)
```
