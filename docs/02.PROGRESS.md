...
## Rebuild processed dataset (remapped masks) + validate

### 1) Run the preprocessing script (from repo root)

> Use **one** of the following commands (depending on how you run it):

```bash
# Option A: run as a module (preferred)
uv run python -m agrovision_core.data.prepare_dataset

# Option B: run by file path (if module import fails)
uv run python agrovision_core/src/agrovision_core/data/prepare_dataset.py
````

---

### 2) Validate masks are contiguous (this confirms remapping worked)

```bash
# Train masks must be exactly [0..13]
uv run python -c "import numpy as np; print(np.unique(np.load('data/processed/train_masks.npy')))"

# Val masks must be exactly [0..13]
uv run python -c "import numpy as np; print(np.unique(np.load('data/processed/val_masks.npy')))"
```
